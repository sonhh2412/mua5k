...
angular.module('bootstrapLightbox').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('lightbox.html',
    "<div class=modal-body ng-swipe-left=Lightbox.nextImage() ng-swipe-right=Lightbox.prevImage()><button class=close aria-hidden=true ng-click=$dismiss()>×</button><div class=lightbox-nav><div class=btn-group ng-if=\"Lightbox.images.length > 1\"><a class=\"btn btn-xs btn-prev btn-default\" ng-click=Lightbox.prevImage()>‹ Previous</a> <a ng-href={{Lightbox.imageUrl}} target=_blank class=\"btn btn-xs btn-new-tab btn-default\" title=\"Open in new tab\">Open image in new tab</a> <a class=\"btn btn-xs btn-next btn-default\" ng-click=Lightbox.nextImage()>Next ›</a></div></div><div class=lightbox-image-container><div class=lightbox-image-caption><span>{{Lightbox.imageCaption}}</span></div><img ng-if=!Lightbox.isVideo(Lightbox.image) lightbox-src={{Lightbox.imageUrl}}><div ng-if=Lightbox.isVideo(Lightbox.image) class=\"embed-responsive embed-responsive-16by9\"><video ng-if=!Lightbox.isSharedVideo(Lightbox.image) lightbox-src={{Lightbox.imageUrl}} controls autoplay></video><embed-video ng-if=Lightbox.isSharedVideo(Lightbox.image) lightbox-src={{Lightbox.imageUrl}} ng-href={{Lightbox.imageUrl}} iframe-id=lightbox-video class=embed-responsive-item><a ng-href={{Lightbox.imageUrl}}>Watch video</a></embed-video></div></div></div>"
  );
...
angular.module('bootstrapLightbox').provider('Lightbox', function () {
  ...
  this.$get = ['$document', '$injector', '$uibModal', '$timeout', 'ImageLoader',
    ...
    Lightbox.nextImage = function () {
      ...
      if( Lightbox.index >= Lightbox.images.length - 2){
        angular.element(
          document.querySelector('.lightbox-modal .modal-dialog .lightbox-nav')
        ).css({
          'display': 'none'
        });        
      }
