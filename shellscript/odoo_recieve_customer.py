...
#web
print ConfigSectionMap("ClusterOne")
...
port= ConfigSectionMap("ClusterOne")['port']
#odoo
...
def odoo_setCustomer(addr='http://127.0.0.1', port=8069, dbname='mycompany', uid=1, password='admin', data={}):
    time.sleep(1)
    try:
        # data = json.loads(data);
        print data;
        result = xmlrpclib.ServerProxy('%s:%s/xmlrpc/object' % (addr, port)).execute(
            dbname, int(uid), password, 'general.rpc.customer', 'write', [1], data )    
        return result
    except:
        return False
    
#xu ly doc du lieu ve tu queue  
class ThreadedCustomer_worker(threading.Thread):
    def callback(self, ch, method, properties, body):
        # print(" [%s] Received %r" % (method.delivery_tag,body))
        time.sleep( body.count('.') )  
        result = odoo_setCustomer(addr=host, port=port, dbname=db, uid=uid, data=body)
        if result:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.channel.queue_declare(queue='website_customer',durable=True)
            self.channel.exchange_declare(exchange='website_customer_logs',
                             exchange_type='fanout')
            self.channel.queue_bind(exchange='website_customer_logs',
                       queue='website_customer')
            self.channel.basic_qos(prefetch_count=int(prefetchCount))
            self.channel.basic_consume(self.callback, queue='website_customer')
        except:
            pass
