...
def odoo_CloseSession(addr='http://127.0.0.1', port=8069, dbname='mycompany', uid=1, password='admin', data={}):
    time.sleep(1)
    try:
        result = xmlrpclib.ServerProxy('%s:%s/xmlrpc/object' % (addr, port)).execute(
            dbname, int(uid), password, 'general.session.close', 'write', [1], data )    
        return result
    except:
        return False
    
#xu ly doc du lieu ve tu queue  
class ThreadedSessionClose_worker(threading.Thread):
    def callback(self, ch, method, properties, body):
        time.sleep( body.count('.') )  
        result = odoo_CloseSession(addr=host, port=port, dbname=db, uid=uid, data=body)
        if result:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.channel.queue_declare(queue='website_product_session_close',durable=True)
            self.channel.exchange_declare(exchange='website_product_session_close_logs',
                             exchange_type='fanout')
            self.channel.queue_bind(exchange='website_product_session_close_logs',
                       queue='website_product_session_close')
            self.channel.basic_qos(prefetch_count=int(prefetchCount))
            self.channel.basic_consume(self.callback, queue='website_product_session_close')
        except:
            pass
...
for _ in range(3):
    ...
    try:
        td = ThreadedSessionClose_worker()
        td.start()
    except:
        pass
