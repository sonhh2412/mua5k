...
def odoo_setHistory(addr='http://127.0.0.1', port=8069, dbname='mycompany', uid=1, password='admin', data={}):
    time.sleep(1)
    try:
        result = xmlrpclib.ServerProxy('%s:%s/xmlrpc/object' % (addr, port)).execute(
            dbname, int(uid), password, 'general.partner.history', 'write', [1], data )    
        return result
    except:
        return False
    
#xu ly doc du lieu ve tu queue  
class ThreadedHistory_worker(threading.Thread):
    def callback(self, ch, method, properties, body):
        # print(" [%s] Received %r" % (method.delivery_tag,body))
        time.sleep( body.count('.') )  
        result = odoo_setHistory(addr=host, port=port, dbname=db, uid=uid, data=body)
        if result:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.channel.queue_declare(queue='website_login_history',durable=True)
            self.channel.exchange_declare(exchange='website_login_history_logs',
                             exchange_type='fanout')
            self.channel.queue_bind(exchange='website_login_history_logs',
                       queue='website_login_history')
            self.channel.basic_qos(prefetch_count=int(prefetchCount))
            self.channel.basic_consume(self.callback, queue='website_login_history')
        except:
            pass
